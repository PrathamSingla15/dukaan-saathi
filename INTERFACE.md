...
| _TTS engine_ | Veena (only) | TTS is **Veena** (`maya-research/veena-tts`) — Hindi/English/**Hinglish** via the `hubertsiuzdak/snac_24khz` SNAC decoder. Gated HF repo: needs an authorized `HF_TOKEN` + the `snac` package. A synthesis failure degrades to a short silence. |
...
2. Pre-download Whisper **and the default Veena TTS** in the Dockerfile (`bash scripts/download_models.sh`). Veena (`maya-research/veena-tts` + `hubertsiuzdak/snac_24khz`) is **gated** — add an authorized `HF_TOKEN` Space secret so the download succeeds; without it the voice replies stay silent. Set `DUKAAN_VEENA_4BIT=true` on small-GPU Spaces (~2-3 GB) or keep bf16 (~6 GB).
